...
def get_neighbourhood_load_schedule(neighbourhood: list, hourly_prices):
    ...
    for house in neighbourhood:
        ...
        #shiftable_continuous, shiftable_non_continuous, non_shiftable = sort_appliances(house.elAppliance)
        #optimizable.extend(house.elAppliance)


        # for a in non_shiftable:
        #     house_list.append((house.name, a.name))

        # for a in shiftable_non_continuous:
        #     house_list.append((house.name, a.name))
...
def make_neighbourhood(num_houses: int):
    neighbourhood = []
    appliances = make_list()
...
# print(df)
#
# total_hourly_load = pd.DataFrame(df.sum(axis=1), columns=["Load kWh"])
